...
function UpdatePost() {
  const { setComponent, selectedPost } = useStore();
  ...
  const [ingredients, setIngredients] = useState([]); // 빈 배열로 초기화
  const [instructions, setInstructions] = useState([]); // 빈 배열로 초기화

  // 선택된 게시물 데이터를 상태에 초기화
  useEffect(() => {
    if (selectedPost) {
      setTitle(selectedPost.title || "");
      setRecipeDescription(selectedPost.recipeDescription || "");
      setImages(selectedPost.images || []);
      setCookTime(selectedPost.cookTime || "");

      // 재료 배열을 문자열로 변환
      const mappedIngredients =
        selectedPost.ingredients?.map((item) => item.ingredientName) || [];
      setIngredients(mappedIngredients);

      // 조리 과정을 문자열로 변환
      const mappedInstructions =
        selectedPost.instructions?.map((item) => item.instructionDescription) ||
        [];
  ...
  const handleSavePost = async () => {
    ...
    try {
      const response = await updateRecipe(token, recipeId, updatedData);

      if (response.success) {
        alert("레시피가 성공적으로 수정되었습니다!");
        setComponent("postList");
      } else {
        alert(`레시피 수정 실패: ${response.message || "알 수 없는 오류"}`);
      }
    } catch (error) {
